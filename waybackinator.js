...
function fetchArchiveURL(url, callback) {
    ...
    .on('response', response => {
        ...
        if (status >= 300) {
            return callback({error: `archive.org replied with status ${status} ${response.statusMessage}`});
        }
        ...
        .on('end', () => {
            ...
            try {
                // const {available, url} = JSON.parse(body).archived_snapshots.closest;
                const snapshots = JSON.parse(body).archived_snapshots;
                const snapshot = snapshots.closest;
                if (!snapshot || !snapshot.available) {
                    return callback({error: 'That URL is not available via the internet archive API.'});
                }
                return callback({archiveURL: snapshot.url});
            } catch (error) {
                console.error(error);
                return callback({error: `Error occurred processing archive.org response:\n${body}`});
            }
        });
    })
    .on('error', error => {
...
server.listen({host, port}).on('listening', () => console.log(`Listening on ${host}:${port}`));
